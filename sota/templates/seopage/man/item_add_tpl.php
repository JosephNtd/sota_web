                ...
                <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                    <?php if ($type != 'gioi-thieu' && $type != 'lien-he') { ?>
                        <div class="form-group">
                            ...
                            <textarea class="form-control for-seo" name="dataSeo[mota<?= $k ?>]" id="mota<?= $k ?>" rows="3" placeholder="Mô tả ngắn hiển thị trên banner trang con (<?= $k ?>)"><?= htmlspecialchars_decode(@$item['mota' . $k]) ?></textarea>
                        </div>
