		...
		<?php for ($i = 0; $i < $numberPhoto; $i++) {
			...
			<div class="card card-primary card-outline text-sm">
				...
				<div class="card-body">
					...
					?>
						<div class="form-group">
							<label for="link_video<?= $i ?>"><?= $videoLabel ?>:</label>
							<input type="text" class="form-control" name="dataMulti[<?= $i ?>][link_video]" id="link_video<?= $i ?>" <?= (!$isCustomLabel) ? "onchange=\"youtubePreview(this.value,'#loadVideo" . $i . "');\"" : '' ?> placeholder="<?= $videoLabel ?>">
						</div>
