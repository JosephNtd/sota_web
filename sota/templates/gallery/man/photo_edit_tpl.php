    ...
    <form method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <?php if (isset($config[$com][$type][$dfgallery][$val]['cart_photo']) && $config[$com][$type][$dfgallery][$val]['cart_photo'] == true) { ?>
            ...
            <div class="card card-primary card-outline text-sm">
                ...
                <div class="card-body">
                    <?php if (isset($resMau) && count($resMau) > 0) {
                        foreach ($resMau as $k => $v) { ?>
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <?php if (isset($config[$com][$type][$dfgallery][$val]['video_photo']) && $config[$com][$type][$dfgallery][$val]['video_photo'] == true) {
                    $videoLabel = isset($config[$com][$type][$dfgallery][$val]['video_label']) ? $config[$com][$type][$dfgallery][$val]['video_label'] : 'Video';
                    $isCustomLabel = isset($config[$com][$type][$dfgallery][$val]['video_label']);
                ?>
                    <div class="form-group">
                        <label for="link_video"><?= $videoLabel ?>:</label>
                        <input type="text" class="form-control" name="data[link_video]" id="link_video" <?= (!$isCustomLabel) ? "onchange=\"youtubePreview(this.value,'#loadVideo');\"" : '' ?> placeholder="<?= $videoLabel ?>" value="<?= @$item['link_video'] ?>">
                    </div>
                    <?php if (!$isCustomLabel) { ?>
                        <div class="form-group">
