...
function save_item()
{
	...
	if ($id && $act != 'save_copy') {
		...
		// Xử lý video local khi CẬP NHẬT bài viết
		if (isset($_FILES['taptin'])) {
            $file_video = $func->uploadName($_FILES['taptin']["name"]);
            if ($video = $func->uploadImage("taptin", ".mp4|.MP4|.mov|.avi", UPLOAD_NEWS, $file_video)) {
                $data['taptin'] = $video;
                // Tìm video cũ và xóa khỏi thư mục máy tính
                $row = $d->rawQueryOne("select id, taptin from #_news where id = ? and type = ? limit 0,1", array($id, $type));
                if (!empty($row['taptin'])) $func->delete_file(UPLOAD_NEWS . $row['taptin']);
            }
        }
		...
		// Xử lý video local khi THÊM MỚI hoặc SAO CHÉP bài viết
		if (isset($_FILES['taptin'])) {
            $file_video = $func->uploadName($_FILES['taptin']["name"]);
            if ($video = $func->uploadImage("taptin", ".mp4|.MP4|.mov|.avi", UPLOAD_NEWS, $file_video)) {
                $data['taptin'] = $video;
